...
export function ReportsContent() {
  const { customers, leads, deals, invoices = [] } = useCRM()
  const [dateRange, setDateRange] = useState("30")
  ...
  const {
    ...
    const pipelineData = pipelineStages.map((stage) => {
      ...
      const stageValue = stageLeads.reduce((sum, l) => {
        const val =
          typeof l.value === "number" ? l.value : Number(l.value ?? 0) || 0
        return sum + val
      }, 0)
  ...
  }, [customers, leads, deals, invoices])
  ...
  const exportReport = () => {
    if (!customers.length && !leads.length && !invoices.length && !deals.length) {
    ...
    const kpiSheetRows = [
      ...
      {
        Metric: "Total Invoice Revenue",
        Value: totalInvoiceRevenue,
      },
      {
    ...
    const pipelineSheetRows = pipelineData.map((p) => ({
      Stage: p.stage,
      "Lead Count": p.count,
      "Potential Value": p.value,
    }))

    const leadSourceSheetRows = leadSourceData.map((s) => ({
      Source: s.source,
      "Lead Count": s.count,
    }))

    const invoicesSheetRows = invoices.map((inv: any) => ({
      "Invoice ID": inv.id,
      ...
      "Tax (%)":
        typeof inv.tax === "number" ? inv.tax : Number(inv.tax ?? 0) || 0,
      "Total (with tax)":
    ...
    const leadsSheetRows = leads.map((l: any) => ({
      "Lead ID": l.id,
      Name: l.name ?? "",
      Status: l.status,
      Stage: l.stage,
    ...
    const kpiSheet = XLSXUtils.json_to_sheet(kpiSheetRows)
    XLSXUtils.book_append_sheet(workbook, kpiSheet, "Summary KPIs")

    const pipelineSheet = XLSXUtils.json_to_sheet(pipelineSheetRows)
    XLSXUtils.book_append_sheet(workbook, pipelineSheet, "Pipeline")

    const leadSourceSheet = XLSXUtils.json_to_sheet(leadSourceSheetRows)
    XLSXUtils.book_append_sheet(workbook, leadSourceSheet, "Lead Sources")

    if (invoicesSheetRows.length) {
      const invoicesSheet = XLSXUtils.json_to_sheet(invoicesSheetRows)
      XLSXUtils.book_append_sheet(workbook, invoicesSheet, "Invoices")
    }

    if (leadsSheetRows.length) {
      const leadsSheet = XLSXUtils.json_to_sheet(leadsSheetRows)
      XLSXUtils.book_append_sheet(workbook, leadsSheet, "Leads")
    }

    XLSXWriteFile(workbook, "crm-report.xlsx")
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      ...
      <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10 max-w-[1600px]">
        ...
        {/* Growth Trends (commented in your code) */}
            ...
            <CardContent className="p-5 sm:p-6 space-y-3">
              <div className="flex items-center justify-between p-4 rounded-2xl bg-gradient-to-br from-slate-50 to-slate-100/50 dark:from-slate-800/50 dark:to-slate-800/30 border border-slate-200/50 dark:border-slate-700/50 hover:border-blue-200 dark:hover:border-blue-900/50 transition-all duration-200">
                ...
                <Badge className="bg-gradient-to-br from-blue-500 to-cyan-600 text-white border-0 shadow-md px-3 py-1 text-sm font-bold">
                  {formatCurrency(
                    totalInvoiceRevenue /
                      (deals.filter((d) => d.status === "won").length || 1),
                  )}
                </Badge>
              </div>
              <div className="flex items-center justify-between p-4 rounded-2xl bg-gradient-to-br from-slate-50 to-slate-100/50 dark:from-slate-800/50 dark:to-slate-800/30 border border-slate-200/50 dark:border-slate-700/50 hover:border-amber-200 dark:hover:border-amber-900/50 transition-all duration-200">
