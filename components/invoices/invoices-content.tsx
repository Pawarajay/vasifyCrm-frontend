...
//   // process.env.NEXT_PUBLIC_BACKEND_URL || "https://vasifycrm-backend.onrender.com"
...
//         const invoiceNumber = invoice.invoiceNumber?.toLowerCase?.() ?? ""
//         const customerName =
//           (invoice as any).customerName?.toLowerCase?.() ?? ""
...
export function InvoicesContent() {
  ...
  // NEW: export all filtered invoices to Excel
    ...
    const rows = filteredInvoices.map((inv) => {
      ...
      const total =
        typeof (inv as any).total === "number"
          ? (inv as any).total
          : Number((inv as any).total ?? 0) || amount + (amount * tax) / 100
